...
实例模式（pmocker run）
  gva-server:8080  ←─── MCP:8081 ←── AI 编辑器
...
| `pmocker_query_progress` | 查询项目进度 | project_id, scope(all\|active\|delayed) | GET /pmocker/schedule/progress | 完成率/里程碑/延迟任务 |
| `pmocker_critical_path` | 关键路径分析 | project_id, view(list\|gantt) | GET /pmocker/schedule/criticalPath | 关键路径链/总工期/浮动 |
| `pmocker_impact_analysis` | 变更影响分析 | change_id 或 from_entity+to_entity | GET /pmocker/change/impact | 受影响需求/任务/成本 |
| `pmocker_risk_matrix` | 风险矩阵评估 | project_id, threshold(可选) | GET /pmocker/risk/matrix | 高/中/低分布 + Top 风险 |
| `pmocker_evm_calc` | 挣值计算 | project_id, as_of_date(可选) | GET /pmocker/cost/evm | PV/EV/AC/SPI/CPI/EAC/VAC |
| `pmocker_query_issues` | 问题查询 | project_id, status, severity, assignee | GET /pmocker/issue/list | 问题列表 + 统计 |
| `pmocker_manage_status` | 状态流转 | entity_type, entity_id, action, comment | POST /pmocker/workflow/transition | 新状态 + 工作流历史 |
...
- MCP 端口 = gva-server 端口 + 1（如 8081）
- 端口冲突处理：若 MCP 端口被占用，逐步 +1 探测（8082、8083...），最多探测 10 个端口，仍冲突则报错并提示用户用 `-p` 指定其他 gva-server 端口
...
- `cli/internal/instance/store.go`：instances 表新增 mcp_port、mcp_token 字段
...
AI 编辑器配置（Claude Code）:
  claude mcp add --transport http pmocker http://localhost:8081/mcp --header "x-token: eyJhbGci..."
```
...
**修改文件**：
- `gva/server/plugin/pmocker/*/seed.yaml`：9 个模块各新增 mcp_dynamic_tools 段
